...
* `map(T)`: an `object` where the value type is `T`. For example, `map(string)`
  is an object where all the values are strings).
...
* `duration`: a `string` denoting a duration of time, such as `"1d"`, `"1h30m"`,
  ...
  `s` (for seconds). Values of descending units can be combined to add their
  values together; `"1h30m"` is the same as `"90m"`.
