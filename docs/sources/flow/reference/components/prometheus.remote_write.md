...
`Unknown reference ID` refers to a Prometheus metric that has a reference ID which is not recognized by the remote write endpoint. This can happen for a few reasons, such as:

  - The metric was generated by a Prometheus exporter that is not compatible with the remote write endpoint.
  - The metric was generated by a Prometheus exporter that is using a newer version of the Prometheus protocol than the remote write endpoint supports.
  - The metric was generated by a Prometheus exporter that is using a different naming convention than the remote write endpoint expects.

 When a remote write endpoint encounters an unknown reference ID, it will drop the metric. This can lead to gaps in your monitoring data, so it is important to troubleshoot the issue if you see a significant number of unknown reference IDs.
{{% /admonition %}}
