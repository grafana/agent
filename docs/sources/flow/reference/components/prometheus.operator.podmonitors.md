...
PodMonitors may reference secrets for authenticating to targets to scrape them. In these cases, the secrets will be loaded and refreshed only when the PodMonitor itself is updated, or when this component refreshes its' internal state, which happens on a 5 minute refresh cycle.
