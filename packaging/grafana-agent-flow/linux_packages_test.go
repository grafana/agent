...
func EnvironmentTestDataFolderPermissions(t *testing.T, env Environment) {
	// Installing should create /var/lib/grafana-agent-flow, assign it to the
	// grafana-agent-flow user and grafana-agent group, and set its permissions
	// to 0770.
	res := env.Install()
	require.Equal(t, 0, res.ExitCode, "installation failed")
...
func RPMEnvironment(t *testing.T, pool *dockertest.Pool) Environment {
	...
	container := environmentContainer(
		...
		fmt.Sprintf("../../dist/grafana-agent-flow-0.0.0-1.%s.rpm", runtime.GOARCH),
	)

	return Environment{
		Install: func() ExecResult {
			filename := fmt.Sprintf("/tmp/grafana-agent-flow-0.0.0-1.%s.rpm", runtime.GOARCH)
			return containerExec(t, container, "rpm", "-i", filename)
		},
		Uninstall: func() ExecResult {
			return containerExec(t, container, "rpm", "-e", "grafana-agent-flow")
		},
...
func DEBEnvironment(t *testing.T, pool *dockertest.Pool) Environment {
	...
	container := environmentContainer(
		...
		fmt.Sprintf("../../dist/grafana-agent-flow-0.0.0-1.%s.deb", runtime.GOARCH),
	)

	return Environment{
		Install: func() ExecResult {
			filename := fmt.Sprintf("/tmp/grafana-agent-flow-0.0.0-1.%s.deb", runtime.GOARCH)
			return containerExec(t, container, "dpkg", "--force-confold", "-i", filename)
		},
		Uninstall: func() ExecResult {
			return containerExec(t, container, "dpkg", "-r", "grafana-agent-flow")
		},
		ExecScript: func(script string) ExecResult {
			return containerExec(t, container, "/bin/bash", "-c", script)
		},
