...
provided, no report is generated. A text report will be generated by default.
If a file extension of html is provided, an html report will be generated.
...
func convert(r io.Reader, fc *flowConvert) error {
	...
	if fc.report != "" {
		reportType := convert_diag.Text
		fmt.Println(filepath.Ext(fc.report))
		if filepath.Ext(fc.report) == convert_diag.HTML {
			reportType = convert_diag.HTML
		}

		err = diags.GenerateReport(reportType, fc.report)
		if err != nil {
			return err
		}
	}
