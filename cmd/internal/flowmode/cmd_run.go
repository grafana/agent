...
`,
	...
	cmd.Flags().BoolVar(&r.authEnabled, "server.auth.enabled", r.authEnabled, "Enable HTTP authentication")
	cmd.Flags().StringVar(&r.authUsername, "server.auth.username", r.authUsername, "Username for HTTP authentication")
	cmd.Flags().StringVar(&r.authPasswordFile, "server.auth.password-file", r.authPasswordFile, "File location of password used in HTTP authentication")
...
func (fr *flowRun) Run(configFile string) error {
	...
	var authMiddleware func(next http.Handler) http.HandlerFunc
	if fr.authEnabled {
		password, err := os.ReadFile(fr.authPasswordFile)
		if err != nil {
			return fmt.Errorf("failed to read password file %s: %s", fr.authPasswordFile, err)
		}

		fr.authPassword = strings.TrimSpace(string(password))

		authMiddleware = func(next http.Handler) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				// skip auth for in-memory connections
				if r.RemoteAddr == "memory" {
					next.ServeHTTP(w, r)
					return
				}
				...
				if ok && username == fr.authUsername && password == fr.authPassword {
					next.ServeHTTP(w, r)
					return
				}

				level.Warn(l).Log("msg", "unauthenticated request", "addr", r.RemoteAddr, "path", r.RequestURI)
	...
	clusterAuth := cluster.Authentication{
		Enabled:  fr.authEnabled,
		Username: fr.authUsername,
		Password: fr.authPassword,
	}

	clusterer, err := cluster.New(l, reg, fr.clusterEnabled, fr.httpListenAddr, fr.clusterAdvAddr, fr.clusterJoinAddr, clusterAuth)
	if err != nil {
	...
	// HTTP server
	{
		...
		r.Handle("/metrics", authMiddleware(promhttp.Handler()))
		if fr.enablePprof {
			r.PathPrefix("/debug/pprof").Handler(authMiddleware(http.DefaultServeMux))
		}
		r.PathPrefix("/api/v0/component/{id}/").Handler(authMiddleware(f.ComponentHandler()))

		// Register routes for the clusterer.
		cr, ch := clusterer.Node.Handler()
		r.PathPrefix(cr).Handler(authMiddleware(ch))
		...
		r.Handle("/-/reload", authMiddleware(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		fa := api.NewFlowAPI(f)
		fa.RegisterRoutes(path.Join(fr.uiPrefix, "/api/v0/web"), r, authMiddleware)
		...
		ui.RegisterRoutes(fr.uiPrefix, r, authMiddleware)
