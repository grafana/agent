  ...
  pipelines.linux('Create Linux build image') {
    ...
    steps: [{
      ...
      commands: [
        ...
        'docker buildx build --build-arg="GO_RUNTIME=golang:1.24.2-bullseye" --push --platform linux/amd64,linux/arm64 -t grafana/agent-build-image:$IMAGE_TAG ./build-image',
      ],
    }],
    volumes: [{
      name: 'docker',
  ...
  pipelines.linux('Create Linux build image for boringcrypto') {
    ...
    steps: [{
      ...
      commands: [
        ...
        'docker run --rm --privileged multiarch/qemu-user-static --reset -p yes',
        'docker buildx create --name multiarch --driver docker-container --use',
        'docker buildx build --build-arg="GO_RUNTIME=mcr.microsoft.com/oss/go/microsoft/golang:1.24.2-bullseye" --push --platform linux/amd64,linux/arm64 -t grafana/agent-build-image:$IMAGE_TAG ./build-image',
      ],
    }],
    volumes: [{
      name: 'docker',
