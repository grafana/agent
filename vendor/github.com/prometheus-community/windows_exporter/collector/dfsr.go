...
// DFSRConfig configures and instantiates a new dfsr collector.
type DFSRConfig struct {
	EnabledCollectors string
}

// Name implements Config.
func (c *DFSRConfig) Name() string { return "dfsr" }

// RegisterFlags implements Config.
func (c *DFSRConfig) RegisterFlags(ka *kingpin.Application) {
	ka.Flag("collectors.dfsr.sources-enabled", "Comma-seperated list of DFSR Perflib sources to use.").Default("connection,folder,volume").StringVar(&c.EnabledCollectors)
}

// Build implements Config.
func (c *DFSRConfig) Build() (Collector, error) {
	return NewDFSRCollector(c)
}
...
func NewDFSRCollector(c *DFSRConfig) (Collector, error) {
	...
	dfsrCollector.perfCounters = perfCounters
...
// PerfCounterNames implements the Collector interface.
func (c *DFSRCollector) PerfCounterNames() []string {
	return c.perfCounters
}
