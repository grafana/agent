...
// SMTPConfig configures and instantiates a new smtp collector.
...
func (c *SMTPConfig) Name() string { return "smtp" }

// RegisterFlags implements Config.
func (c *SMTPConfig) RegisterFlags(ka *kingpin.Application) {
	ka.Flag(
	...
	).Default(".+").StringVar(&c.ServerWhitelist)
	ka.Flag(
	...
	).StringVar(&c.ServerBlacklist)
}

// Build implements Config.
func (c *SMTPConfig) Build() (Collector, error) { return NewSMTPCollector(c) }
...
func NewSMTPCollector(c *SMTPConfig) (Collector, error) {
	...
	return &SMTPCollector{
		...
		serverWhitelistPattern: regexp.MustCompile(fmt.Sprintf("^(?:%s)$", c.ServerWhitelist)),
		serverBlacklistPattern: regexp.MustCompile(fmt.Sprintf("^(?:%s)$", c.ServerBlacklist)),
	}, nil
}

// PerfCounterNames implements Collector.
func (c *SMTPCollector) PerfCounterNames() []string { return []string{"SMTP Server"} }
