...
// LogicalDiskConfig configures and instantiates a new logical_disk collector.
...
func (c *LogicalDiskConfig) Name() string { return "logical_disk" }

// RegisterFlags implements Config.
func (c *LogicalDiskConfig) RegisterFlags(ka *kingpin.Application) {
	ka.Flag(
...
func (c *LogicalDiskConfig) Build() (Collector, error) { return NewLogicalDiskCollector(c) }
...
func NewLogicalDiskCollector(c *LogicalDiskConfig) (Collector, error) {
	...
	return &LogicalDiskCollector{
		...
		volumeWhitelistPattern: regexp.MustCompile(fmt.Sprintf("^(?:%s)$", c.VolumeWhitelist)),
		volumeBlacklistPattern: regexp.MustCompile(fmt.Sprintf("^(?:%s)$", c.VolumeBlacklist)),
	}, nil
}

// PerfCounterNames implements Collector.
func (c *LogicalDiskCollector) PerfCounterNames() []string {
