...
// MSSQLConfig configures and instantiates a new mssql collector.
...
func (c *MSSQLConfig) Name() string { return "mssql" }

// RegisterFlags implements Config.
func (c *MSSQLConfig) RegisterFlags(ka *kingpin.Application) {
	ka.Flag(
		"collectors.mssql.classes-enabled",
		"Comma-separated list of mssql WMI classes to use.").
		Default(mssqlAvailableClassCollectors()).StringVar(&c.EnabledCollectors)

	ka.Flag(
		"collectors.mssql.class-print",
		"If true, print available mssql WMI classes and exit.  Only displays if the mssql collector is enabled.",
	).BoolVar(&c.PrintCollectors)
}

// Build implements Config.
func (c *MSSQLConfig) Build() (Collector, error) { return NewMSSQLCollector(c) }
...
// NewMSSQLCollector ...
func NewMSSQLCollector(c *MSSQLConfig) (Collector, error) {

	const subsystem = "mssql"

	enabled := expandEnabledChildCollectors(c.EnabledCollectors)
...
// PerfCounterNames implements Collector.
func (c *MSSQLCollector) PerfCounterNames() []string { return c.perfCounters }
...
func (c *MSSQLCollector) Collect(ctx *ScrapeContext, ch chan<- prometheus.Metric) error {
	...
	enabled := expandEnabledChildCollectors(c.config.EnabledCollectors)
