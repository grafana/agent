...
// NetworkConfig configures and instantiates a new net collector.
...
func (c *NetworkConfig) Name() string { return "net" }

// RegisterFlags implements Config.
func (c *NetworkConfig) RegisterFlags(ka *kingpin.Application) {
	ka.Flag(
...
func (c *NetworkConfig) Build() (Collector, error) { return NewNetworkCollector(c) }
...
// NewNetworkCollector ...
func NewNetworkCollector(c *NetworkConfig) (Collector, error) {
	const subsystem = "net"
	...
	return &NetworkCollector{
		...
		nicWhitelistPattern: regexp.MustCompile(fmt.Sprintf("^(?:%s)$", c.NICWhitelist)),
		nicBlacklistPattern: regexp.MustCompile(fmt.Sprintf("^(?:%s)$", c.NICBlacklist)),
	}, nil
}

// PerfCounterNames implements Collector.
func (c *NetworkCollector) PerfCounterNames() []string {
