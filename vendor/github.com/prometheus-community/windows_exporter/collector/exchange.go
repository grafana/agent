...
// ExchangeConfig configures and instantiates a new exchange collector.
...
func (c *ExchangeConfig) Name() string { return "exchange" }

// RegisterFlags implements Config.
func (c *ExchangeConfig) RegisterFlags(ka *kingpin.Application) {
	ka.Flag(
	...
	).Default("").StringVar(&c.Enabled)
}

// Build implements Config.
func (c *ExchangeConfig) Build() (Collector, error) { return newExchangeCollector(c) }
...
func newExchangeCollector(cfg *ExchangeConfig) (Collector, error) {
	...
	if cfg.Enabled == "" {
		...
		for _, collectorName := range strings.Split(cfg.Enabled, ",") {
...
func (c *exchangeCollector) PerfCounterNames() []string {
	return []string{
