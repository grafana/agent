...
// IISConfig configures and instantiates a new iis collector.
...
func (c *IISConfig) Name() string { return "iis" }

// RegisterFlags implements Config.
func (c *IISConfig) RegisterFlags(ka *kingpin.Application) {
	ka.Flag(
	...
	).Default(".+").StringVar(&c.SiteWhitelist)
	ka.Flag(
	...
	).Default(".+").StringVar(&c.AppWhitelist)
	ka.Flag(
...
func NewIISCollector(c *IISConfig) (Collector, error) {
	...
	buildIIS := &IISCollector{
		...
		siteWhitelistPattern: regexp.MustCompile(fmt.Sprintf("^(?:%s)$", c.SiteWhitelist)),
		siteBlacklistPattern: regexp.MustCompile(fmt.Sprintf("^(?:%s)$", c.SiteBlacklist)),
		...
		appWhitelistPattern: regexp.MustCompile(fmt.Sprintf("^(?:%s)$", c.AppWhitelist)),
		appBlacklistPattern: regexp.MustCompile(fmt.Sprintf("^(?:%s)$", c.AppBlacklist)),
	}
...
func (c *IISCollector) PerfCounterNames() []string { return nil }
