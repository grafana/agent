...
type AttrActionSettings struct {
	Actions []*AttrActionKeyValue `river:"action,block,optional"`
}

// Convert converts args into the upstream type.
func (args *AttrActionSettings) Convert() *attraction.Settings {
	if args == nil {
		return nil
	}

	var res attraction.Settings
	for _, action := range args.Actions {
		res.Actions = append(res.Actions, *action.Convert())
	}
	return &res
}
...
// Convert converts args into the upstream type.
func (args *AttrActionKeyValue) Convert() *attraction.ActionKeyValue {
	if args == nil {
		return nil
	}
	...
	return &attraction.ActionKeyValue{
		...
		Action:        attraction.Action(args.Action),
	}
}
