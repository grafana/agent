...
type WriteStorage struct {
	...
	queues            map[string]*promremote.QueueManager
	samplesIn         *ewmaRate
	flushDeadline     time.Duration
	scraper           promremote.ReadyScrapeManager
	quit              chan struct{}
...
func NewWriteStorage(logger log.Logger, reg prometheus.Registerer, flushDeadline time.Duration, sm promremote.ReadyScrapeManager) *WriteStorage {
	if logger == nil {
		logger = log.NewNopLogger()
	}
	rws := &WriteStorage{
		queues:            make(map[string]*promremote.QueueManager),
...
func (rws *WriteStorage) ApplyConfig(conf *config.Config) error {
	...
	newQueues := make(map[string]*promremote.QueueManager)
	newHashes := []string{}
	for _, rwConf := range conf.RemoteWriteConfigs {
		...
		c, err := promremote.NewWriteClient(name, &promremote.ClientConfig{
			URL:              rwConf.URL,
		...
		newQueues[hash] = promremote.NewQueueManager(
