...
func metricQuery(metricName string) string {
	return fmt.Sprintf("%s%s{test_name='scrape_prom_metrics_old_module_git'}", promURL, metricName)
}
...
func assertHistogramData(t *testing.T, query string, expectedMetric string) {
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		...
		if assert.NotEmpty(c, metricResponse.Data.Result) {
			...
			assert.Equal(c, metricResponse.Data.Result[0].Metric.TestName, "scrape_prom_metrics_old_module_git")
