...
const query = "http://localhost:9009/prometheus/api/v1/query?query=avalanche_metric_mmmmm_0_23{job='prometheus.scrape.scrap_prom_metrics23'}"
...
func TestScrapPromMetrics(t *testing.T) {
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		...
		if assert.NotEmpty(c, metricResponse.Data.Result) {
			assert.Equal(c, metricResponse.Data.Result[0].Metric.Name, "avalanche_metric_mmmmm_0_23")
			assert.Equal(c, metricResponse.Data.Result[0].Metric.Job, "prometheus.scrape.scrap_prom_metrics23")
		}
