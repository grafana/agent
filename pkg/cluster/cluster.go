...
func New(log log.Logger, reg prometheus.Registerer, clusterEnabled bool, name, listenAddr, advertiseAddr, joinAddr, discoverPeers string, rejoinInterval time.Duration, advertiseInterface []string) (*Clusterer, error) {
	// Standalone node.
	if !clusterEnabled {
	...
	if advertiseInterface != nil {
		gossipConfig.AdvertiseInterfaces = advertiseInterface
	}
