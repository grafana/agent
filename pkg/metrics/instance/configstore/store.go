...
type Store interface {
	// List gets the list of shared names.
	List(ctx context.Context) ([]string, error)

	// Get gets an individual shared by name.
	Get(ctx context.Context, key string) (instance.Config, error)

	// Put applies a new instance Config to the store.
	// If the shared already exists, created will be false to indicate an
	// update.
	Put(ctx context.Context, c instance.Config) (created bool, err error)

	// Delete deletes a shared from the store.
...
// WatchEvent is returned by Watch. The Key is the name of the shared that was
