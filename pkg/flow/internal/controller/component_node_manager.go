...
func (m *ComponentNodeManager) ComputeCustomComponentDependencies(declareNode *DeclareNode) []CustomComponentDependency {
	m.mut.Lock()
	defer m.mut.Unlock()
	...
	if deps, ok := m.customComponentDependencies[declareNode.label]; ok {
		...
		dependencies = m.findCustomComponentDependencies(declareNode.Declare())
...
func (m *ComponentNodeManager) findCustomComponentDependencies(declare *Declare) []CustomComponentDependency {
